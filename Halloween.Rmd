...
```{r}
# Fit linear model
fit <- lm(incidence_rate ~ haunted_houses, data = halloween_zombie_outbreak)

# Create predicted values
halloween_zombie_outbreak$pred <- predict(fit)
...
plot_ly(halloween_zombie_outbreak, 
  ...
  layout(title = "Haunted Houses vs. Incidence Rate with Regression",
...
plot_ly(halloween_zombie_outbreak, x = ~haunted_houses, y = ~incidence_rate,
        type = "scatter", mode = "markers", marker = list(size = 10, color = "purple")) %>%
  add_histogram(x = ~haunted_houses, yaxis = "y2") %>%
  layout(
    yaxis2 = list(overlaying = "y", side = "right", title = "Histogram of Haunted Houses")
  )
```
